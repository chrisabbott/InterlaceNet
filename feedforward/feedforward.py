...
def feedforward(X, architecture, weights, biases, return_label=False):
  ...
  # Output layer index
  output_index = len(architecture) - 2
...
def backpropagation(y, architecture, activations, outputs, weights, biases):
  # Calculate the error in the output layer
  error = np.multiply(activations[-1] - y, sigmoid(outputs[-1], inverse=True))
...
def test_network(X, y, architecture, weights, biases):
  acc = []
