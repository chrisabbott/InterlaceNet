'''
    -> Output weights are averaged prior to training (Output layer squashed to (40x10))
'''
...
def main():
  ...
  supermatrix_weight_a, supermatrix_weight_b = staggered_init(weights1[0], weights2[0], init="zeros")
  weights3.append(interweave(supermatrix_weight_a, supermatrix_weight_b))
  biases3.append(np.vstack((biases1[0], biases2[0])))
  weights3.append(interweave(weights1[1], weights2[1]))
  ...
  # Get accuracy prior to training
  pre_accuracy3 = ff.test_network(test_X, test_y, architecture3, weights3, biases3) * 100
  ...
  for epoch in range(epochs):
    ...
    for batch in range(60000 // batch_size):
      ...
      gradients3 = ff.backpropagation(batch_y, architecture3, activations3, outputs3, weights3, biases3)
    ...
    # Get accuracy post epoch
    train3_acc = ff.test_network(train_X, train_y, architecture3, weights3, biases3) * 100
    test3_acc = ff.test_network(test_X, test_y, architecture3, weights3, biases3) * 100
